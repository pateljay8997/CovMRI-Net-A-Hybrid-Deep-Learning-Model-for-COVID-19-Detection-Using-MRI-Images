...
def find_model_file(directory='D:\Btech_project\Covid\EfficientNet'):
...
results_dir = 'D:\Btech_project\Covid\EfficientNet'
...
if model_path is None:
    ...
    print("Please ensure that the EfficientNet model file is in the correct directory.")
    sys.exit(1)
...
try:
    ...
    print(f"Error loading the EfficientNet model: {e}")
    sys.exit(1)
...
generator = datagen.flow_from_dataframe(
    ...
    x_col='filename',
    y_col='class',
    target_size=(224, 224),  # Adjust if EfficientNet requires a different input size
    batch_size=32,
...
# Evaluate the model
print("Evaluating the EfficientNet model...")
...
with open(os.path.join(results_dir, 'efficientnet_evaluation_results.txt'), 'w') as f:
    f.write(f"EfficientNet Model file: {model_path}\n")
...
print("EfficientNet evaluation completed. Results saved in 'evaluation_results' directory.")